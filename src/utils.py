...
class Plotter:
    ...
    def make_bar(self):
        ...
        num_orgs = self.data.groupby("ACO_name_1").agg({"org_name": "count"}).reset_index()
        top_num_orgs = num_orgs.sort_values(by = "org_name", ascending = False).iloc[0:10,]

        bar_chart = alt.Chart(top_num_orgs).mark_bar().encode(
            alt.X("ACO_name_1"),
            alt.Y("org_name"),
